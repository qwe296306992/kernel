			...
			const struct usb_device_id *id)
{
	...
	/* 对于ME363 4G模块, 在option_probe函数加入下面这一段代码
	 * 防止USB网卡被加载成为USB串口
	 */
	dev_info(&serial->dev->dev, "idVendor=%x, idProduct=%x, bInterfaceNumber=%d\r\n",
	...
	if (serial->dev->descriptor.idVendor == ZTE_VENDOR_ID &&
			serial->dev->descriptor.idProduct == 0x1476 &&
			iface_desc->bInterfaceNumber == 5)
		return -ENODEV;

#if 1 //Added by Quectel
	if (serial->dev->descriptor.idVendor == cpu_to_le16(0x2C7C)) {
		//EM05-CE
		if ((serial->dev->descriptor.idProduct == cpu_to_le16(0x0125)) &&
			(iface_desc->bInterfaceNumber >= 4))
			return -ENODEV;
	}
#endif
