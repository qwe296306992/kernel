...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (val == 89) {
		m89or101 = TRUE;
		ts->gtp_change_x2y = TRUE;
		ts->gtp_x_reverse = FALSE;
		ts->gtp_y_reverse = TRUE;
	} else if (val == 101) {
		m89or101 = FALSE;
		ts->gtp_change_x2y = TRUE;
		ts->gtp_x_reverse = TRUE;
		ts->gtp_y_reverse = FALSE;
	} else if (val == 911) {
		m89or101 = FALSE;
		bgt911 = TRUE;
		ts->gtp_change_x2y = FALSE;
		ts->gtp_x_reverse = FALSE;
		ts->gtp_y_reverse = FALSE;
	} else if (val == 9110) {
		m89or101 = FALSE;
	...
	if (of_property_read_bool(np, "touchscreen-inverted-x"))
		ts->gtp_x_reverse = !ts->gtp_x_reverse;
	if (of_property_read_bool(np, "touchscreen-inverted-y"))
		ts->gtp_y_reverse = !ts->gtp_y_reverse;
	if (of_property_read_bool(np, "touchscreen-swapped-x-y"))
		ts->gtp_change_x2y = !ts->gtp_change_x2y;

	if (of_property_read_string(np, "goodix-ts-name", &ts->name))
		ts->name = goodix_ts_name;
